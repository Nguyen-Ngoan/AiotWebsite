        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={
                'whitespace-nowrap py-2 px-3 border-b-2 text-sm font-medium transition-colors ' +
                (isActive
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          >
            <option value="draft">Draft</option>ika
...
function DescriptionTab({
  ...
  return (
    <div className="space-y-2">
      ...
      <textarea
        className="mt-1 block w-full min-h-[220px] rounded-md border border-gray-300 px-3 py-2 text-sm font-mono shadow-sm focus:border-blue-500 focus:ring-blue-500"
        value={descriptionHtml}
        onChange={(e) => setDescriptionHtml(e.target.value)}
        placeholder="<p>Mô tả chi tiết sản phẩm, bảng thông số kỹ thuật...</p>"
      />
      <p className="text-xs text-gray-500">
        Bạn có thể dán HTML đã format từ editor (Tiptap) vào đây.
      </p>
    </div>
  );
}
  ...
  const addItem = () => {
    const nextId =
      bundleItems.length > 0
        ? Math.max(...bundleItems.map((i) => i.id)) + 1
        : 1;
    setBundleItems([
      ...bundleItems,
      { id: nextId, childProductId: '', quantity: 1 },
    ]);
  };
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-600">
        Cấu hình các sản phẩm con thuộc bundle này. Backend hiện tại chưa lưu
        cấu trúc này trong Firestore, nhưng UI đã sẵn sàng để sau này nối với
        model <code>ProductItem</code>.
      </p>
      ...
      <div className="space-y-3">
        {bundleItems.map((item) => (
          <div
            key={item.id}
            className="flex flex-col md:flex-row md:items-end gap-2 rounded-md border border-gray-200 p-3"
          >
            <div className="flex-1">
              <label className="block text-xs font-medium text-gray-700">
                ID sản phẩm con (childProductId)
              </label>
              <input
                type="text"
                className="mt-1 block w-full rounded-md border border-gray-300 px-2 py-1 text-xs shadow-sm focus:border-blue-500 focus:ring-blue-500"
                value={item.childProductId}
                onChange={(e) =>
                  updateItem(item.id, 'childProductId', e.target.value)
                }
              />
            </div>
            <div className="w-28">
            ...
            <button
              ...
              className="self-start md:self-auto inline-flex items-center rounded-md border border-red-300 bg-white px-2 py-1 text-xs font-medium text-red-600 shadow-sm hover:bg-red-50"
            >
              Xóa
            </button>
          </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <textarea
          className="mt-1 block w-full min-h-[160px] rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
          value={gallery.join('\n')}
    ...
    <div className="space-y-4">
      <div>
        ...
        <input
          ...
          placeholder="Tiêu đề hiển thị trên tab & search result"
        />
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Mô tả ngắn cho SEO (160 ký tự)..."
        />
      </div>
      ...
      <div>
        ...
        <input
          ...
          placeholder="URL ảnh chia sẻ social"
        />
      </div>
    </div>
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
            value={stepModelUrl}
            onChange={(e) => setStepModelUrl(e.target.value)}
            placeholder="https://..."
          />
        </div>
        <div>
          ...
          <input
            ...
            className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:ring-blue-500"
            value={stlFilesUrl}
            onChange={(e) => setStlFilesUrl(e.target.value)}
            placeholder="https://..."
          />
        </div>
      </div>
...
export default function ProductForm({
  ...
  const [stockTracking, setStockTracking] = useState<boolean>(
    initialData?.stock_tracking ?? true
  );
  const [stockQty, setStockQty] = useState<string>(
    initialData?.stock_qty?.toString() || ''
  );
  const [minOrderQty, setMinOrderQty] = useState<string>(
    initialData?.min_order_qty?.toString() || '1'
  );
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-5">
          ...
          {activeTab === 'basic' && (
            <BasicInfoTab
              ...
              setStockTracking={setStockTracking}
              stockQty={stockQty}
          ...
          {activeTab === 'docs' && (
            <DocsTab
              ...
              setSchematicUrl={setSchematicUrl}
              stepModelUrl={stepModelUrl}
              setStepModelUrl={setStepModelUrl}
              stlFilesUrl={stlFilesUrl}
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 space-y-3">
          ...
          <div className="text-sm text-gray-700 space-y-1">
            <div>
              ...
              {title || <span className="text-gray-400">Chưa nhập</span>}
            </div>
            <div>
              <span className="font-medium">Slug:</span>{' '}
              {slug || <span className="text-gray-400">Sẽ tự sinh từ tên</span>}
            </div>
            <div>
            ...
            <div>
              ...
              {stockQty || <span className="text-gray-400">Chưa đặt</span>}
            </div>
            <div>
              <span className="font-medium">Tags:</span>{' '}
              {tags || <span className="text-gray-400">Chưa có tag</span>}
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 space-y-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="inline-flex flex-1 justify-center items-center rounded-md border border-transparent bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-blue-700 disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {isSubmitting ? 'Đang lưu...' : submitButtonText}
            </button>
            <button
              type="button"
          ...
          <p className="text-xs text-gray-500">
            Sau khi lưu thành công, bạn sẽ được chuyển về trang danh sách sản
            phẩm DIY.
          </p>
        </div>
      </div>
